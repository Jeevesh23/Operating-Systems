...
{
public:
    int burst_time, cbt, arrival_time, index, turn_around_time, waiting_time;
};
...
int main()
{
    ...
    while (length(n, processes) > 0)
    {
        ...
        for (p = 0; p < n && processes[p].burst_time < 0; p++)
            ;
        for (int i = p; i < n; i++)
            if (processes[i].burst_time >= 0 && processes[i].arrival_time <= time && processes[i].burst_time <= processes[p].burst_time)
            {
                idle = false;
                if (processes[i].burst_time < processes[p].burst_time)
                    p = i;
            }
        if (idle)
        {
    ...
    cout << "\nID | ARRIVAL TIME | BURST TIME | WAITING TIME | TURN AROUND TIME |\n";
    cout.fill('-');
    cout.width(78);
    cout << std::right << " " << endl;
    cout.fill(' ');
